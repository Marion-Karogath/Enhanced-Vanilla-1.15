...
public class Shrub1Structure extends EnhancedVanillaElements.ModElement {
	public Shrub1Structure(EnhancedVanillaElements instance) {
		super(instance, 243);
	}
	...
	public void init(FMLCommonSetupEvent event) {
				...
				if ((random.nextInt(1000000) + 1) <= 550000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("enhancedvanilla", "small_shrub1"));
						if (template == null)
							return false;
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("enhancedvanilla:chaparral")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
